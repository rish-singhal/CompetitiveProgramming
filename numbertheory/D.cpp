...
//	cout<<(rr<0?0:pvv[rr])<<" --- "<<(ll<0?0:pvv[ll])<<endl;
	LL an = ((rr<0?0:pvv[rr]) - (ll<0?0:pvv[ll]))%M;
	if(an<0) an+=M;
	return an;
}
...
void solve(){
    ...
    //set<LL> s;
    LL cub = 0;
    while(!isOverflow(cub*cub,cub) && cub*cub*cub<=n) cub++;
    cub--;
	for(LL i=2; !isOverflow(i*i,i) && i*i*i<=n;i++){
	  	if(sfree[i]) continue;
	  	LL j = (i*i)%M;
	  	LL nn = n/(i*i); 
	  	while(nn>cub){
 ...
 // cout<<" ---  "<<an<<endl;
  for(LL i=1; !isOverflow(i*i,i) && i*i*i<=n;i++){
  	LL l = (n/(i+1))+1;
  	LL r = (n/i);
  	LL v = i;
  	LL fl =(LL) floor(sqrt(l+.0));
  	LL fr =(LL) floor(sqrt(r+.0));
...
int main(){
  ...
  for(LL i=2;i<=1e6+5;i++){
  	LL j = i*i*i; 
  	LL cnt = 3;
  	while(j<=1e18+5 && cnt<=63 ){
  		if(isqrt(j)==-1) vv.pb(j);
  		if(!isOverflow(j,i) && j*i<=((1e18+5))){
  			j*=i;
  			cnt++;
  		}
  		else break;
